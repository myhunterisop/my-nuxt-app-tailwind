...
seo.setSeo({
  title: 'Пользователи | MyApp',
  description: 'Список пользователей приложения MyApp'
})
...
const loadUsers = async () => {
  ...
  isLoaded.value = true
  try {
    const data = await $fetch<unknown[]>('https://jsonplaceholder.typicode.com/users')
    users.value = z.array(UserSchema).parse(data)
  } catch (e: any) {
    error.value = e.errors ? e.errors.map((err: any) => err.message).join('; ') : (e.message || 'Ошибка загрузки пользователей')
    users.value = []
  } finally {
